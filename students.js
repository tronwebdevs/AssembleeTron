...
router.post('/iscriviti', isAuthenticated, (req, res) => {
    if (req.body.ora1 && req.body.ora2 && req.body.ora3 && req.body.ora4) {
        let connection;
        let labs;
        let selectedLabs = [req.body.ora1, req.body.ora2, req.body.ora3,  req.body.ora4];
        mysql.createConnection(mysqlCredentials).then((conn) => {
            connection = conn;
            return connection.query('SELECT `ID` AS `labID`, `Ora1` AS `labPostiOra1`, `Ora2` AS `labPostiOra2`, `Ora3` AS `labPostiOra3`, `Ora4` AS `labPostiOra4` FROM `Progetti`');
        }).then((rows) => {
            let promiseArray = [];
            rows.forEach((el) => {
                if (selectedLabs.indexOf(el.labID) != -1) {
                    promiseArray.push(connection.query({
                        sql: 'SELECT COUNT(?) AS `subs` FROM  `Iscritti` WHERE `Ora1`=?',
                        values: [ el.labID, el.labID ]
                    }));
                    promiseArray.push(connection.query({
                        sql: 'SELECT COUNT(?) AS `subs` FROM  `Iscritti` WHERE `Ora2`=?',
                        values: [ el.labID, el.labID ]
            ...
            labs.forEach((lab, index) => {
                if ( (lab.labPostiOra1 - results[index * 4] - 1) > 0) {
                    let error = new Error('Posti esauriti per un Laboratorio');
